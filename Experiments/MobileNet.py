...
def main(model_config):
    ...
    Model = models.mobilenet_v2(pretrained=model_config.pre_train).to(device)
    preprocess = transforms.Compose([
        transforms.Resize(256),
        transforms.CenterCrop(224),
    ])

    Model.classifier[1] = nn.Linear(1280, 10, device=device)
    ...
    if model_config.pre_train:
        url = "https://huggingface.co/James0323/FashionMnist/resolve/main/MobileNet_45.pth"
    ...
    for epoch in range(100):
        ...
        with open('trainMobile.txt', 'a') as f:
        ...
        if report['accuracy'] > best_acc:
            torch.save(Model.state_dict(), 'MobileNet_' + str(epoch + 1) + '.pth')
        ...
        with open('validationMobile.txt', 'a') as f:
